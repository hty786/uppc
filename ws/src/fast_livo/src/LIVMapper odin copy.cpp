...
{
  //
  std::cout << "\n[DESTRUCTOR] Class is being destroyed. Saving map..." << std::endl;
  savePCD();
  //
}
...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  try_declare.template operator()<bool>("pcd_save.pcd_save_en", true);
  try_declare.template operator()<bool>("pcd_save.colmap_output_en", false);
  try_declare.template operator()<double>("pcd_save.filter_size_pcd", 0.05);

  //修改点：使其读取avia.yaml文件的pcd_save_path路径
  try_declare.template operator()<std::string>("pcd_save.pcd_save_path", "/tmp/");
  node->get_parameter("pcd_save.pcd_save_path",pcd_save_path);
  RCLCPP_INFO(node->get_logger(), "PCD Save Path Configured to: %s", pcd_save_path.c_str());
  ...
  this->node->get_parameter_or("pcd_save.pcd_save_path", pcd_save_path, std::string("/home/ek/pcd2pgm_ros2/maps/"));
...
{
  //调试
  std::cout << "\n[DEBUG] Trying to save PCD..." << std::endl;
  std::cout << "[DEBUG] pcd_save_en: " << (pcd_save_en ? "true" : "false") << std::endl;
  std::cout << "[DEBUG] Point Size: " << pcl_wait_save->points.size() << std::endl;
  std::cout << "[DEBUG] Intensity Point Size: " << pcl_wait_save_intensity->points.size() << std::endl;
  std::cout << "[DEBUG] Interval: " << pcd_save_interval << std::endl;
  std::cout << "[DEBUG] Path: " << pcd_save_path << std::endl;
  ...
  if (pcd_save_en && (pcl_wait_save->points.size() > 0 || pcl_wait_save_intensity->points.size() > 0) && pcd_save_interval < 0) 
  {
    ...
    std::string valid_path = pcd_save_path;
    std::cout << GREEN << "Raw point cloud data saved to: ..." << std::endl;

    if (!valid_path.empty() && valid_path.back() != '/') {
        valid_path += "/";
    }
    else
    {
      std::cout << RED << "[ERROR] Save condition not met!" << RESET << std::endl;
    }

    // 建议改成 scans.pcd，方便后续脚本处理，也可以用原来的名字
    std::string raw_points_dir = valid_path + "scans.pcd"; 
    std::string downsampled_points_dir = valid_path + "scans_filtered.pcd";
    //到此
...
void LIVMapper::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubOdomAftMapped)
{
  ...
  br->sendTransform(geometry_msgs::msg::TransformStamped(createTransformStamped(transform, odomAftMapped.header.stamp, "odom", "aft_mapped")));
